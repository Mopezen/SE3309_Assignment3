...
WHERE duration > (4 * (SELECT AVG(duration) FROM prescription));
UPDATE medicalStaff
...
UPDATE patient
SET medicalStaffID = (SELECT medicalStaffID FROM medicalstaff WHERE specialization IN ('Orthopaedics','Maternity','Anaesthetics') ORDER BY RAND() LIMIT 1)
WHERE patientID = 50;
